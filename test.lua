...
function main()
	while is_run do
		...
		if isConnected() == 1 and status_torg == 1 then -- проверка наличия соеденения с сервером и поподания в торговое окно
			table_show:SetCaption("Bot (Test) - ожидаем данные")
			ds_num = ds:Size()
		...
		if is_torg then -- торговля
			table_show:SetCaption("Bot (Test) - торговля")
...
-- место для размещения торговой логики	
			...
			if pos_number == pos_number then
				if profit >= conf.profit then -- закрываем позицию, если достигнут профит 
					...
					if pos_number == 0 then   -- вход в позицию
						if open_long then
							trans_id, trans_msg = SendMarketOrder_test(conf, conf.sec_code, "B", conf.trade_lots)
							pos_number = conf.trade_lots
							last_buy_price = carrent_price
						elseif 	open_short then
							trans_id, trans_msg = SendMarketOrder_test(conf, conf.sec_code, "S", conf.trade_lots)
							pos_number = -conf.trade_lots
							last_sell_price = carrent_price
						else
						
						end
					elseif pos_number >= conf.trade_lots then -- выход из лонга
						if trend_up then
							if close_long and profit > 0 then
								trans_id, trans_msg = SendMarketOrder_test(conf, conf.sec_code, "S", abs_pos_number)
								pos_number = 0
								last_sell_price = carrent_price
							end
						else
					...
					elseif pos_number <= -conf.trade_lots then --выход из шорта
						if trend_down then
							if close_short and profit > 0 then
								trans_id, trans_msg = SendMarketOrder_test(conf, conf.sec_code, "B", abs_pos_number)
								pos_number = 0
								last_buy_price = carrent_price
							end		
						else 
		...
		sleep(100)            -- обрабатываем цикл с задержкой 0.1сек.
    end
end 
