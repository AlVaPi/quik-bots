...
function main()
	while is_run do
		...
		if isConnected() == 1 and status_torg == 1 then -- проверка наличия соеденения с сервером и поподания в торговое окно
			table_show:SetCaption("Bot (SAR) - ожидаем данные")
			ds_num = ds:Size()
			...
			sar_candles = getNumCandles(conf.tag_sar)
			sar_0, sar_0_num, sar_0_legend = getCandlesByIndex(conf.tag_sar, 0, 0, sar_candles)		
			is_torg = ds:Size() > 0 and bb_candles > 2 and sar_candles > 0
		end
 		
		if is_torg then -- торговля
			table_show:SetCaption("Bot (SAR) - торговля")
			bb_center = bb_0[bb_0_num-1].close
			bb_high = bb_1[bb_1_num-1].close
			bb_low = bb_2[bb_2_num-1].close
			bb_trend_up = bb_center = bb_0[bb_0_num-1].close > bb_center = bb_0[bb_0_num-2].close
			bb_trend_down = bb_center = bb_0[bb_0_num-1].close < bb_center = bb_0[bb_0_num-2].close
			if bb_trend_up then 
				bb_trend_ind = "bb trend up"
			elseif bb_trend_down then
				bb_trend_ind = "bb trend down"
			else
				bb_trend_ind = "no bb trend"
			end			
			sar_value = sar_0[sar_0_num-1].close
			sar_trend_up = carrent_price > sar_value
			sar_trend_down = carrent_price < sar_value
			if sar_trend_up then 
				sar_trend_ind = "sar trend up"
			elseif sar_trend_down then
				sar_trend_ind = "sar trend down"
			else
				sar_trend_ind = "no sar trend"
			end			
			abs_pos_number = math.abs (pos_number)			
			open_long = sar_trend_up and bb_trend_up
			open_short = sar_trend_down and bb_trend_down
			if open_long then 
				ls_ind = "open long"
			...
			UpdateTableRow(table_show, 3, "BB", "", MathRound(bb_center), bb_trend_ind, "", "")			
			UpdateTableRow(table_show, 4, "SAR", "", MathRound(sar_value), sar_trend_ind, "", "")
...
function OnTableEvent (t_id, msg, par1, par2)
	if msg == QTABLE_LBUTTONDBLCLK and par1 ==4 then
		if par2 == 1 then
			message ('Bot (SAR) --> Стоп скрипт вручную', 2)
			OnStop()
		end
		if par2 == 6 then
			message ('Bot (SAR) --> Закрываем позиции вручную и стоп', 2)
			CloseAllPos()
			OnStop()
		end
	end
